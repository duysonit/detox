...
class FormPage {
    ...
    async verifyEditMemberPage(formData) {
        await expect(this.editMemberHeader).toHaveText(`Edit Member ${baseData.getId(formData.member)}`);
        await this.verifyFormLabels();
        ...
        await expect(this.emailInput).toHaveText(baseData.getEmail(formData.member));
    ...
    async selectCalendarDate(weekday, day, month, year) {
        while (await utilities.softTextAssertion(element(by.id('undefined.header.title')),
            `${month} ${year}`) === false) {
    ...
    async selectDatePickerDate(day, month, year) {
        if (device.getPlatform() === 'ios') {
            ...
            await element(by.type('android.widget.EditText')).atIndex(2).tapReturnKey();
            await element(by.type('android.widget.EditText')).atIndex(1).typeText(day);
            await element(by.type('android.widget.EditText')).atIndex(1).tapReturnKey();
            await element(by.type('android.widget.EditText')).atIndex(0).typeText(baseData.getMonth(month));
            await element(by.type('android.widget.EditText')).atIndex(0).tapReturnKey();
        }
    }
